...
AUTO_FIT_ZOOM_WINDOW_S = (-0.010, 0.010)
AUTO_FIT_ZOOM_WINDOW_MS = (
...
def _estimate_peak_time_s(relative_time_s, delta_tau_mpa):
    ...
    search_mask = (time_s >= AUTO_FIT_ZOOM_WINDOW_S[0]) & (time_s <= AUTO_FIT_ZOOM_WINDOW_S[1])
    search_time = time_s[search_mask]
    search_signal = smooth_signal[search_mask]
    if search_signal.size == 0:
        raise ValueError(f"No samples inside peak search window {AUTO_FIT_ZOOM_WINDOW_S}.")
...
def _build_fit_problem(relative_time_s, delta_tau_mpa, peak_info):
    ...
    analysis_mask = (time_s >= AUTO_FIT_ZOOM_WINDOW_S[0]) & (time_s <= AUTO_FIT_ZOOM_WINDOW_S[1])
    analysis_indices = np.flatnonzero(analysis_mask)
    if analysis_indices.size == 0:
        raise ValueError(f"No samples found inside zoom window {AUTO_FIT_ZOOM_WINDOW_S}.")
...
class CZMFitterView(tk.Toplevel):
    ...
    def load_event(self, event_idx):
        ...
        if "czm_parms" in self.event:
            ...
            if has_pick_arrivals:
                self.x_lim_min, self.x_lim_max = AUTO_FIT_ZOOM_WINDOW_S
    ...
    def update_plot(self, event=None):
        ...
        if selected_label and isinstance(pick_arrivals, dict) and selected_label in (pick_arrivals.get("delta_tau_mpa_by_label") or {}):
            ...
            t_ms = t * 1000.0
            display_xlim = AUTO_FIT_ZOOM_WINDOW_MS
